		...
		for k, v in attrs.items():
			if isinstance(v, Field):
				...
				if v.primary_key:
					...
					fields.append(k)
		if not primaryKey:
			raise RuntimeError('Primaary key not found.')
	...
	def getValueOrDefault(self, key):
		...
		if value is None:
			...
			if field.default is not None:
				value = field.default() if callable(self.default) else field.default
...
u = User(id=12345, name='snow', email='test@orm.org', password='my-pwd')
await u.save()
#await u.find(12345)
